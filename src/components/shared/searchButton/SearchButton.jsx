...
const SearchButton = () => {
  const {adult,child,selectedOption,room,formattedCheckinDate,formattedCheckoutDate,currency,latitude,longitude,location,history,destType,destid,orderBy,toggle} = useApp (); 

  const {recentHandler} = useRecentHandler();

  const {t} = useTranslation();

  const orderParam = encodeURIComponent(JSON.stringify(orderBy))

  const searchLink = `/search?city=${location || history.recent[0]?.city}&room=${parseInt(room)}&latitude=${latitude || history.recent[0]?.lat}&longitude=${longitude || history.recent[0]?.lng}&currency=${currency}&locale=${localStorage.getItem('i18nextLng')}&checkoutdate=${formattedCheckoutDate}&checkindate=${formattedCheckinDate}&adult=${parseInt(adult)}&children=${parseInt(child)}${child > 0 ? `&children_ages=${selectedOption}`:''}&dest_id=${destid || history.recent[0]?.dest_id}&dest_type=${destType||  history.recent[0]?.dest_type}&order_by=${orderParam || history.recent[0]?.order_by}`; 
